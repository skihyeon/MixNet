...
class TextLoss(nn.Module):
    ...
    def loss_calc_flux(pred_flux, gt_flux, weight_matrix, mask, train_mask):
        ...
        angle_loss = (1 - torch.cosine_similarity(pred_flux, gt_flux, dim=1)).clamp(min=0)
        angle_loss = angle_loss[mask].mean()

        # print(f"gt_flux stats: min={gt_flux.min()}, max={gt_flux.max()}, mean={gt_flux.mean()}")
        # print(f"pred_flux stats: min={pred_flux.min()}, max={pred_flux.max()}, mean={pred_flux.mean()}")
    ...
    def forward(self, input_dict, output_dict, eps=None):
        ...
        #         if name == 'point_loss':
        #             print(f"py_preds length: {len(py_preds)}")
        #             if len(py_preds) > 1:
        #                 non_empty_preds = [p for p in py_preds[1:] if p.numel() > 0]
        #                 if non_empty_preds:
        #                     print(f"py_preds[1:] stats: min={min([p.min() for p in non_empty_preds])}, max={max([p.max() for p in non_empty_preds])}")
        #                 else:
        #                     print("All py_preds[1:] are empty")
        #             print(f"gt_tags[inds] stats: min={gt_tags[inds].min()}, max={gt_tags[inds].max()}")
