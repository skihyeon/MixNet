...
gt_dir = './data/kor/Test/gt'
fid_path = '{}/Eval_My_{}_{}.txt'.format(eval_result_dir, args.tr, args.tp)
    ...
    for ext in extensions:
        ...
        if os.path.exists(json_path):
            with open(json_path, 'r', encoding='utf-8') as f:
                gt = json.load(f)
            return gt
    raise FileNotFoundError(f"No JSON file found for {gt_id} with extensions {extensions}")

def detection_filtering(detections, groundtruths, threshold=0.5):
    for gt in groundtruths['images'][0]['fields']:
        vertices = gt['boundingPoly']['vertices']
        gt_x = [v['x'] for v in vertices] 
        gt_y = [v['y'] for v in vertices]
...
for i, input_id in enumerate(tqdm(allInputs)):
    if (input_id != '.DS_Store') and (input_id != 'Pascal_result.txt') and (input_id != 'Deteval_result.txt'):
        ...
        local_sigma_table = np.zeros((len(groundtruths['images'][0]['fields']), len(detections)))
        local_tau_table = np.zeros((len(groundtruths['images'][0]['fields']), len(detections)))
        local_size_table = np.zeros(len(detections))
        
        flag = True
        for gt_id, gt in enumerate(groundtruths['images'][0]['fields']):
            vertices = gt['boundingPoly']['vertices'] 
            gt_x = [v['x'] for v in vertices]
            gt_y = [v['y'] for v in vertices]
            pts = np.array(np.stack([gt_x, gt_y])).T
            # print(pts)
            pts = pts.astype(np.int32) ## contour 함수에 입력되려면 int32 or float32 형식이어야 함
            if pts.shape[0] > 2:  # 점의 개수가 2개 이상인 경우에만 면적 계산
                ...
                print(f"Skipping contour with {pts.shape[0]} points")
